...
impl VBuffer {
    pub unsafe fn new(paddr: PhyAddr, size: usize) -> Result<VBuffer> {
        VBuffer::with_flags(
            paddr,
            size,
            Flags::READ_WRITE | Flags::NO_EXECUTE
        )
    }
    ...
    pub unsafe fn with_flags(paddr: PhyAddr, size: usize, flags: Flags) -> Result<VBuffer> {
        ...
        let page_count = usize::from(page_end_addr).wrapping_sub(usize::from(base_page_addr))
            / PAGE_SIZE;
...
impl Drop for VBuffer {
    fn drop(&mut self) {
        ...
        let page_count = usize::from(base_page_addr).wrapping_sub(usize::from(last_page_end))
            / PAGE_SIZE;
