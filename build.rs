...
fn main() {
    ...
    compiler
        .compile(&out_dir, "liblambix.a")
        .expect("error when compiling assembly files");
...
impl<'c> Compiler<'c> {
    ...
    pub fn create_archive(
        ...
        archive_process.arg("crus").arg(output.to_str().unwrap());
        ...
        let status = archive_process
            .status()
            .expect("can't start archive process");
        if status.success() {
            Ok(())
        } else {
    ...
    fn compile_file(
        ...
        if nasm_compiler
            .status()
            .expect("can't start nasm command")
            .success()
        {
    ...
    fn compile_files_in_directory<'a, 'b>(
        dir: &Path,
        out_dir: &Path,
        out_files: &mut Vec<PathBuf>,
    ) -> Result<(), Error> {
